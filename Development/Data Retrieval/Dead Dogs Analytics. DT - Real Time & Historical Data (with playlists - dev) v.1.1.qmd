...
```{r}
# BEWARE! Before to run this section be sure to delete the file .httr-oauth under the path
# D:\PC3\Operation Ultra\09 - R\Projects\Dead Dogs Analytics\Dead Dogs Analytics ShinyDashboard
...
videos_hist <- base::as.data.frame(youtube_json[["rows"]]) %>% 
  ...
  dplyr::transmute(views_date = as.Date(V1)
                  ,video_id = V2
                  ,video_daily_views = V3
                  ) |> 
  tidyr::complete(views_date = seq.Date(min(views_date), lubridate::today(), by = "day")) |> 
  tidyr::fill(video_id) |> 
  dplyr::mutate(video_daily_views = as.integer(base::ifelse(is.na(video_daily_views), 0, video_daily_views)))
